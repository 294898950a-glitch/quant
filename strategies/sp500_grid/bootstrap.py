"""sp500_grid bootstrap — 一次性初始化 holdout 池 + state.json。
...
::
    ...
    python -m strategies.sp500_grid.bootstrap [--data-dir PATH]
                                              [--n-pools N]
                                              [--force]
"""
...
def bootstrap(
    ...
    if verbose:
        print(
            f"[bootstrap] validating prices parquet at {prices_parquet}",
            file=sys.stderr,
        )
    ...
    if verbose:
        print(
            f"[bootstrap] {len(oos_dates)}/{total_rows} rows fall in OOS "
    ...
    if verbose:
        print(
            f"[bootstrap] OK. {len(oos_dates)} OOS dates split into "
            f"{n_pools} pools of sizes {pool_sizes}",
            file=sys.stderr,
        )
        for i, p in enumerate(pools):
            print(
                f"[bootstrap]   pool {i}: {len(p)} dates ({p[0]} -> {p[-1]})",
                file=sys.stderr,
            )
        print(
        ...
        print(
            ...
            "python -m strategies.sp500_grid.orchestrator_main --live",
            file=sys.stderr,
        )
...
def _build_argparser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="bootstrap",
        description=(
            "One-shot setup for sp500_grid self-loop: cuts holdout pools "
