"""csi500_grid bootstrap 测试 —— 全部用 tmp_path + 合成 parquet。"""
...
from strategies.csi500_grid.bootstrap import (
    DEFAULT_YAML_PATH,
...
def test_bootstrap_creates_files(tmp_path: Path):
    ...
    parquet = tmp_path / "raw" / "510500_daily.parquet"
    ...
    assert sp["version"] == 1
    assert sp["strategy"] == "csi500_grid"
    assert sp["n_pools"] == 4
