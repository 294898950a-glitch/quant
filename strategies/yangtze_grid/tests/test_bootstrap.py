"""yangtze_grid bootstrap 测试 —— 全部用 tmp_path + 合成 parquet。"""
...
from strategies.yangtze_grid.bootstrap import (
    DEFAULT_YAML_PATH,
...
def test_bootstrap_creates_files(tmp_path: Path):
    ...
    parquet = tmp_path / "raw" / "600900_daily.parquet"
    ...
    assert sp["version"] == 1
    assert sp["strategy"] == "yangtze_grid"
    assert sp["n_pools"] == 4
