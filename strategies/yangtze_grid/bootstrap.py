"""yangtze_grid bootstrap — 一次性初始化 holdout 池 + state.json。
...
::
    ...
    python -m strategies.yangtze_grid.bootstrap [--data-dir PATH]
                                            [--n-pools N]
                                            [--force]
"""
...
def bootstrap(
    ...
    if verbose:
        print(f"[yangtze grid bootstrap] validating yaml at {yaml_path}", file=sys.stderr)
    _validate_yaml(yaml_path)

    if verbose:
        print(
            f"[yangtze grid bootstrap] validating prices parquet at {prices_parquet}",
            file=sys.stderr,
        )
    ...
    if verbose:
        print(
            f"[yangtze grid bootstrap] {len(oos_dates)}/{total_rows} rows fall in OOS "
    ...
    if verbose:
        print(
            f"[yangtze grid bootstrap] OK. {len(oos_dates)} OOS dates split into "
            f"{n_pools} pools of sizes {pool_sizes}",
            file=sys.stderr,
        )
        for i, p in enumerate(pools):
            print(
                f"[yangtze grid bootstrap]   pool {i}: {len(p)} dates ({p[0]} -> {p[-1]})",
                file=sys.stderr,
            )
        print(
            f"[yangtze grid bootstrap] sealed_pools.json -> {sealed_pools_path}",
            file=sys.stderr,
        )
        print(
            f"[yangtze grid bootstrap] state.json        -> {state_path}",
            file=sys.stderr,
        )
        print(
            "[yangtze grid bootstrap] OK. Now you can run: "
            "python -m strategies.yangtze_grid.orchestrator_main --live",
            file=sys.stderr,
        )
...
def _build_argparser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="bootstrap",
        description=(
            "One-shot setup for yangtze_grid self-loop: cuts holdout pools "
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"[yangtze grid bootstrap] ERROR: {exc}", file=sys.stderr)
        return 1
    return 0
