"""shenhua_grid verifier 适配器 —— 把 GridResult 转成 BacktestResult。
...
为了让 shenhua_grid 能跨进框架，本模块的 :func:`run_backtest` 既兼容此约定，
...
GridConfig）。shenhua 与 sp500/csi500 唯一差别在于：(1) 价格 parquet 路径，
(2) yaml 路径 / strategy 名 / 标的代码 (601088)。
...
- ``snapshots`` 参数被故意忽略（grid 不需要，但 cb-style 签名留着以便
  人工调试）；本函数自己读 ``data/shenhua_grid/raw/601088_daily.parquet``。
...
def _load_prices(prices_parquet: Path = DEFAULT_PRICES_PARQUET) -> pd.DataFrame:
    """读取 shenhua grid 价格 parquet。"""
    if not prices_parquet.exists():
        raise FileNotFoundError(
            f"shenhua_grid prices parquet not found at {prices_parquet}"
        )
