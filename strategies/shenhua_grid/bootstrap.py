"""shenhua_grid bootstrap — 一次性初始化 holdout 池 + state.json。
...
::
    ...
    python -m strategies.shenhua_grid.bootstrap [--data-dir PATH]
                                            [--n-pools N]
                                            [--force]
"""
...
DEFAULT_DATA_DIR = _REPO_ROOT / "data" / "shenhua_grid"
DEFAULT_YAML_PATH = _HERE / "tunable_space.yaml"
DEFAULT_PRICES_PARQUET = _REPO_ROOT / "data" / "shenhua_grid" / "raw" / "601088_daily.parquet"
DEFAULT_OOS_SPLIT_DATE = "20220101"
DEFAULT_N_POOLS = 4
DEFAULT_SEED = 42
SCHEMA_VERSION = 1
STRATEGY_NAME = "shenhua_grid"
...
def bootstrap(
    ...
    if verbose:
        print(f"[shenhua grid bootstrap] validating yaml at {yaml_path}", file=sys.stderr)
    _validate_yaml(yaml_path)

    if verbose:
        print(
            f"[shenhua grid bootstrap] validating prices parquet at {prices_parquet}",
            file=sys.stderr,
        )
    ...
    if verbose:
        print(
            f"[shenhua grid bootstrap] {len(oos_dates)}/{total_rows} rows fall in OOS "
    ...
    if verbose:
        print(
            f"[shenhua grid bootstrap] OK. {len(oos_dates)} OOS dates split into "
            f"{n_pools} pools of sizes {pool_sizes}",
            file=sys.stderr,
        )
        for i, p in enumerate(pools):
            print(
                f"[shenhua grid bootstrap]   pool {i}: {len(p)} dates ({p[0]} -> {p[-1]})",
                file=sys.stderr,
            )
        print(
            f"[shenhua grid bootstrap] sealed_pools.json -> {sealed_pools_path}",
            file=sys.stderr,
        )
        print(
            f"[shenhua grid bootstrap] state.json        -> {state_path}",
            file=sys.stderr,
        )
        print(
            "[shenhua grid bootstrap] OK. Now you can run: "
            "python -m strategies.shenhua_grid.orchestrator_main --live",
            file=sys.stderr,
        )
...
def _build_argparser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="bootstrap",
        description=(
            "One-shot setup for shenhua_grid self-loop: cuts holdout pools "
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"[shenhua grid bootstrap] ERROR: {exc}", file=sys.stderr)
        return 1
    return 0
