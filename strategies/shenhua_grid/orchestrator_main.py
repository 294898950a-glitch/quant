"""shenhua_grid orchestrator CLI 入口 —— 复用 cb_redemption.Orchestrator 类。

设计要点
--------
- 不复制 Orchestrator 类。直接 import + DI 把 shenhua_grid 的 verifier 注进去。
...
::
    ...
    python -m strategies.shenhua_grid.orchestrator_main \\
...
from strategies.shenhua_grid.verifier import run_backtest as grid_verifier
...
def _build_argparser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="shenhua_grid_orchestrator",
        description="shenhua_grid self-loop daemon (layer 7) — DI on cb.Orchestrator",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.dry_run:
        # Default: tmp dir so real data/shenhua_grid/ + yaml are untouched.
        if data_dir is None:
            data_dir = Path(tempfile.mkdtemp(prefix="shenhua_grid_dryrun_"))
        else:
    ...
    orch = Orchestrator(
        ...
        # 唯一 DI 是 verifier —— 用 shenhua_grid 自己的回测,而不是 cb 的。
