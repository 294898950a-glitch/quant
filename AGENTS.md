...
1. **OS 文件系统层 (cross-AI 真硬)**:
   - `scripts/framework_watch_daemon.py` 后台 daemon, 监控 reports/ data/ docs/
   - 任意文件修改 (任何 AI / 任何编辑器 / 任何用户) 都触发 framework_doc_check
   - 错了写到 `logs/framework_watch.log` + desktop notification (notify-send)
   - **不依赖任何特定 AI 工具的 hook 机制**
   - 启动: `nohup python3 scripts/framework_watch_daemon.py > /dev/null 2>&1 &`
...
4. **Git pre-commit hook (最后关)**: `scripts/install_pre_commit_hook.sh` 装,
   commit 时跑 framework_preflight, 防错的进仓库.
...
任何 AI Write / Edit 受管路径文件后, daemon 会触发 framework_doc_check.py.
