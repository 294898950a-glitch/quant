...
def _resolve_data_path(data_root: str | Path, relative: str) -> Path:
    ...
    # Also try stripping leading "data/" prefix
    if rel.parts[0] == "data":
...
def _extract_gap_closing_events(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for _code, grp in df.groupby("ts_code"):
        ...
        # Force-close any still-open gap at end of data
        if in_gap:
            last = grp.iloc[-1]
...
def _bin_index(gap_pct: float, bin_size: float) -> int:
    """Map a gap percentage to a bin index."""
    if gap_pct <= 0:
        return 0
...
def _build_bin_cdf(
    events: pd.DataFrame, gap_bin_size: float, events_df: pd.DataFrame | None = None
    ...
    If events_df is provided, it overrides events (used by simulation).
    """
    ...
    bin_cdf: dict[int, tuple[np.ndarray, int]] = {}
...
def _percentile_in_bin(hold_days: int, sorted_days: np.ndarray, total_events: int) -> float:
    """Compute the percentile of hold_days within the distribution.

    Returns value in [0, 100].  If hold_days exceeds max observed, returns 100.
    If hold_days is before min observed, returns 0.
    """
    if total_events == 0:
        return 0.0
    # Count how many events closed in <= hold_days
    pos = int(np.searchsorted(sorted_days, hold_days + 1, side="right"))
    return round(pos / total_events * 100.0, 2)


def _build_rolling_cdf(
    events: pd.DataFrame, gap_bin_size: float, up_to_date: pd.Timestamp,
) -> dict[int, tuple[np.ndarray, int]]:
    """Build CDF using only events whose exit_date <= up_to_date."""
...
def _compute_metrics(trades: list[dict[str, Any]], total_pnl: float) -> dict[str, Any]:
    ...
    # Max drawdown from cumulative PnL curve (sorted by exit_date)
...
def _write_artifacts(
    ...
    # Adoption criteria: best candidate has positive excess in all three periods
    # and max drawdown is not materially worse than baseline.
    ...
    # Remove trades list from metrics to keep summary compact
    for key in ("train", "test", "yr2020"):
        if "trades" in summary[key]:
            summary[key]["trade_count"] = summary[key].pop("trade_count", len(summary[key].get("trades", [])))
...
# Grid-search sweep ranges (hardcoded per executor convention)
...
def main() -> int:
    ...
    # Extract all gap-closing events (needed for rolling CDF during simulation)
    # Use entire raw dataset to build the full event history, then
    # the _build_rolling_cdf will filter by date during simulation.
    try:
    ...
    for mhd in _SWEEP_MIN_HOLD_DAYS:
        for pct in _SWEEP_PERCENTILE:
            for binsz in _SWEEP_BIN_SIZE:
                ...
                # Score by test excess return for best candidate selection
    ...
    # Select best candidate's params for artifact writing
