"""一次性拉取博时标普500ETF (513500.SH) 日线，落盘到 parquet。

输出: data/sp500_grid/raw/513500_daily.parquet
列: date (str YYYYMMDD), open, high, low, close, vol, amount
    open/high/low/close/amount = float64; vol = int64

akshare 接口尝试顺序:
    1. ak.fund_etf_hist_em(symbol="513500", ...)
    2. ak.fund_etf_hist_sina(symbol="sh513500")  (fallback)
失败重试 3 次，每次间隔 0.5s。
"""
...
OUT_DIR = REPO_ROOT / "data" / "sp500_grid" / "raw"
OUT_PATH = OUT_DIR / "513500_daily.parquet"
...
def _try_sina() -> pd.DataFrame:
    ...
    df = ak.fund_etf_hist_sina(symbol="sh513500")
    return df
