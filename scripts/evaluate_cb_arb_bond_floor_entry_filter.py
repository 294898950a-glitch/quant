...
decay_period_factor, effective_max_hold_days) as the exit baseline for every
variant.  Grid-searches over bond_floor_threshold and min_days_below.
"""
...
def _load_cb_basic() -> pd.DataFrame:
    ...
    # pandas StringDtype → object for consistent handling
    for c in ("ts_code",):
        if hasattr(df[c], "str"):
...
def _compute_bond_floor(
    ...
    """
    # Merge basic info onto daily prices
    ...
    if active_mask.any():
        ...
        # Number of full-year coupons remaining (excluding current partial year)
...
def _build_entry_mask(
    ...
    *consecutive* days at-or-below threshold, the CB is re-allowed.
    ...
    allowed: dict[str, list[tuple[pd.Timestamp, bool]]] = {}
    ...
    for ts, grp in bf.groupby("ts_code"):
        ...
        for i in range(n):
            if np.isnan(ratios[i]):
                # If we can't compute ratio, suppress
                status[i] = False
                below_streak = 0
        ...
        for d, s in zip(dates, status):
            allowed.setdefault(d, {})[ts] = s

    # Apply to gap_ranks
    ranks = gap_ranks.copy()
    ranks["_allow_entry"] = ranks.apply(
        lambda r: allowed.get(r["trade_date"], {}).get(r["ts_code"], True),
        axis=1,
    )
    return ranks
...
# ────────────────────────────────────────────────────────────────────────
#  Duration-adaptive exit simulation (mirrors the accepted executor)
# ────────────────────────────────────────────────────────────────────────
...
def _simulate_duration_adaptive(
    ...
    # Cumulative excess compound (simplified: sum of PnL, since baselines are on
    # the same scale)
...
def _mask_diagnostics(
    ...
    # Days suppressed by year (for sanity check)
...
def main() -> int:
    ...
    # cumulative_excess_compound_improvement >= 5pp vs baseline
    # max_single_year_drawdown < 15% (here using max_drawdown in PnL units as proxy)
    # any_holdout_year_sharpe > 0.5 (test period)
    # trade_count remains above 50% of baseline
    criteria_met = (
        excess_test > 0
        and excess_train > 0
        and excess_2020 > 0
        and trade_count_retention > 50.0
        # Not checking absolute max_drawdown PnL here because the scale depends
        # on the raw PnL, but we check directionally: filtered DD >= baseline DD
