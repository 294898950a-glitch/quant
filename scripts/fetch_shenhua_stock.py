"""一次性拉取中国神华 (601088.SH) 日线，落盘到 parquet。

输出: data/shenhua_grid/raw/601088_daily.parquet
列: date (str YYYYMMDD), open, high, low, close, vol, amount
    open/high/low/close/amount = float64; vol = int64

akshare 接口尝试顺序:
    1. ak.stock_zh_a_hist(symbol="601088", adjust="qfq", ...)        # 东财, 主路径
    2. ak.stock_zh_a_daily(symbol="sh601088", adjust="qfq")          # 新浪, 带前缀, fallback
失败重试 3 次，每次间隔 0.5s。

注意：中国神华 2007-10 上市，start=20071009 是首日；这里用 20070101 让 akshare 自动从首日返回。
"""
...
OUT_DIR = REPO_ROOT / "data" / "shenhua_grid" / "raw"
OUT_PATH = OUT_DIR / "601088_daily.parquet"
...
def _try_sina() -> pd.DataFrame:
    ...
    df = ak.stock_zh_a_daily(symbol="sh601088", adjust="qfq")
    return df
