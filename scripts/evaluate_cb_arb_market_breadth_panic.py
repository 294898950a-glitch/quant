"""Search cb_arb market-breadth panic opportunity protection.

The breadth signal is exogenous to strategy PnL: it uses the previous trading
day's cross-sectional close-to-close CB returns to decide whether date T is a
risk/opportunity-protection day.  T-1 lag is mandatory to avoid same-close
lookahead.
"""
...
def _candidate_grid() -> list[dict[str, Any]]:
    ...
    for drop_threshold, panic_ratio, recovery, hurdle, min_days in itertools.product(
        [-0.03, -0.05, -0.07],
        [0.20, 0.30, 0.40],
        [1, 2, 3],
        [0.05, 0.08, 0.10],
        ...
        rows.append(
            {
                "name": (
                    ...
                    f"h{_fmt_float(hurdle)}_min{min_days}"
                ),
...
def _effective_dates_for_candidate(
    ...
    raw = breadth[threshold_col].astype(float) >= float(candidate["breadth_panic_ratio"])
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("data/cb_arb_market_breadth_panic_2026-05-15"),
    )
    parser.add_argument(
        "--benchmark-csv",
        type=Path,
...
def main() -> int:
    ...
    baseline_results: dict[int, dict[str, Any]] = {}
    print(f"[market-breadth] deriving baseline years={years}", flush=True)
    for year in years:
        result = _run_year(
            ranks,
            args.data_root,
        ...
        print(
            f"[market-breadth] baseline year={year} "
    ...
    print(
        f"[market-breadth] candidates={len(candidates)} years={len(years)} "
    ...
    with ProcessPoolExecutor(
        ...
        for done, fut in enumerate(as_completed(futures), 1):
            ...
            if done % max(1, len(years) * max(1, args.max_workers)) == 0 or done == total_tasks:
                print(f"[market-breadth] tasks_done={done}/{total_tasks}", flush=True)
    ...
    for leave_year in years:
        ...
        print(
            f"[market-breadth] leave={leave_year} top={rows[0]['name']} "
    ...
    print("[market-breadth] summary", json.dumps(run_summary, ensure_ascii=False), flush=True)
    print(f"[market-breadth] wrote {args.output_dir}", flush=True)
    return 0
