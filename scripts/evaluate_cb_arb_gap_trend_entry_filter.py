"""Evaluate gap trend entry filter for cb_arb value-gap switch.

Computes per-candidate rolling gap change (gap_trend) from daily value gap amounts,
builds a boolean entry filter mask (only allow entry when gap_trend < 0 — gap has narrowed),
filters the daily value gap ranks to suppress entries during gap widening phases,
and runs the existing backtester without modifying scoring, sizing, or exit logic.

Called once per window size W. Each call runs baseline (no filter) and the filtered
variant across train, stress_2020, and validate periods, then writes summary.json,
report.yaml, l4_ack.yaml, and diagnostic.yaml.
"""
...
# ── fixed backtest params (do not vary across filter variants) ──────────
...
def declare_data_requirements(
    ...
    data_root_raw = _command_value(command, "--data-root") or "data/cb_arb_concurrent_supervised_20260511_094500"
    data_root = Path(data_root_raw)
    fixed_source = FIXED_SOURCE
    pool_ids = sorted({0, 2, 4, 6, fixed_source})
    return {
        "schema_version": 1,
        "executor": "scripts/evaluate_cb_arb_gap_trend_entry_filter.py",
        "required_files": [
            {
                "path": "data/cb_arb_value_gap_switch_regime-option-entry-gate_2026-05-17/daily_value_gap_amounts.parquet",
                "description": (
                    "Daily value-gap ranks with theoretical_value and "
                    "market_price_mid needed to compute gap_t."
                ),
            },
            {
...
def _build_gap_trend_mask(
    ranks: pd.DataFrame,
    W: int,
) -> pd.DataFrame:
    """Compute per-candidate gap_trend and return boolean entry mask.

    For each candidate (ts_code) on each day:
        gap_t = theoretical_value - market_price_mid
        gap_trend = (gap_t / gap_{t-W} - 1)

    Entry is allowed when gap_trend < 0 (gap has narrowed).
    The first W days for each candidate are always allowed (no prior gap available).
    """
    required_cols = ["ts_code", "trade_date", "theoretical_value", "market_price_mid"]
    available_cols = [c for c in required_cols if c in ranks.columns]

    mask = ranks[["ts_code", "trade_date"]].copy()

    if "theoretical_value" in ranks.columns and "market_price_mid" in ranks.columns:
        gap_t = ranks["theoretical_value"].astype(float) - ranks["market_price_mid"].astype(float)
    else:
        # fallback: use value_gap_amount if theoretical/mid not present
        if "value_gap_amount" in ranks.columns:
            gap_t = ranks["value_gap_amount"].astype(float)
        else:
            # no gap data available, allow all entries
            mask["entry_allowed"] = True
            mask["gap_trend"] = 0.0
            return mask

    mask["gap_t"] = gap_t
    mask = mask.sort_values(["ts_code", "trade_date"]).reset_index(drop=True)

    # per-candidate rolling prior gap
    mask["gap_prior"] = mask.groupby("ts_code")["gap_t"].shift(W)

    # gap_trend = (gap_t / gap_prior) - 1
    mask["gap_trend"] = mask["gap_t"] / mask["gap_prior"] - 1.0

    # entry allowed if gap_trend < 0 (narrowing) or no prior (first W days)
    mask["entry_allowed"] = (mask["gap_trend"] < 0.0) | mask["gap_prior"].isna()

    mask["entry_allowed"] = mask["entry_allowed"].fillna(True)
    mask["gap_trend"] = mask["gap_trend"].fillna(0.0)

    return mask[["ts_code", "trade_date", "entry_allowed", "gap_trend"]]


def _apply_filter(
    ranks: pd.DataFrame, entry_mask: pd.DataFrame
) -> pd.DataFrame:
    """Return ranks with gap-widening candidate-days suppressed."""
    merged = ranks.merge(
        entry_mask[["ts_code", "trade_date", "entry_allowed"]],
        on=["ts_code", "trade_date"],
        how="left",
    )
    # rows that didn't match the mask (shouldn't happen) are kept
    merged["entry_allowed"] = merged["entry_allowed"].fillna(True)
    filtered = merged[merged["entry_allowed"]].drop(columns=["entry_allowed"])
    return filtered
...
def _write_report(
    ...
    report = {
        ...
        "learnings": [
            f"Gap trend entry filter (W={W}) evaluated against train, 2020 stress, and validate periods.",
        ],
        "follow_up_actions": (
            ["seek user approval before promoting the candidate"]
            if adoption_pass
            else ["review why the gap trend filter failed one or more fixed criteria"]
        ),
        "status": "COMPLETE",
        "generated_by": "hermes",
        "generated_at": now,
...
def _write_l4_ack(
    ...
    ack = {
        ...
        "q4_monotonic": {
            "description": (
                "Candidate must improve 2020 stress score "
                "without breaking train score."
            ),
            "answer": (
                f"2020 pass={_score(ft_stress) > -0.10}; "
                f"train pass={_score(ft_train) > 0.10}"
            ),
            "computed_data": {
                "filtered_2020_score": _score(ft_stress),
                "filtered_train_score": _score(ft_train),
            ...
            "computed_at": now,
            "pass": bool(_score(ft_stress) > -0.10 and _score(ft_train) > 0.10),
        },
        "q5_trade_overlap": {
            ...
            "computed_data": {
                "filtered_train_trades": int(ft_train.get("total_trades", 0) or 0),
...
def _write_diagnostic(
    ...
    diagnostic = {
        ...
        "summary": (
            f"Gap trend entry filter {variant_name} "
        ...
        "filtered_metrics": {
            ...
            "train_excess_return": float(ft_train.get("excess_return", 0.0) or 0.0),
            "train_max_drawdown": float(ft_train.get("max_drawdown", 0.0) or 0.0),
            ...
            "validate_total_trades": int(ft_validate.get("total_trades", 0) or 0),
        },
        "checks": checks,
        "warnings": [],
        "errors": [],
...
def main() -> int:
    ...
    parser.add_argument("--test-end", default="20260508")
    parser.add_argument("--output-dir", type=Path, required=True)
    parser.add_argument("--window-size", "-W", type=int, required=True)
    args = parser.parse_args()

    output_dir = args.output_dir
    output_dir.mkdir(parents=True, exist_ok=True)
    W = args.window_size
    variant_name = f"gap_trend_W{W}"
    ...
    # build gap trend entry mask
    entry_mask = _build_gap_trend_mask(ranks_full, W)

    # filtered ranks
    ranks_filtered = _apply_filter(ranks_full, entry_mask)

    params = BASE_PARAMS

    # run baseline (no filter) on all periods
    print(f"[gap_trend_entry_filter] {variant_name} running baseline", flush=True)
    baseline_results: dict[str, dict[str, Any]] = {}
    for period in PERIODS:
    ...
    print(f"[gap_trend_entry_filter] {variant_name} running filtered", flush=True)
    filtered_results: dict[str, dict[str, Any]] = {}
    for period in PERIODS:
    ...
    # write artifacts
    _write_summary(
        output_dir, variant_name, W,
        baseline_results, filtered_results, adoption_pass,
    )
    _write_report(
        output_dir, variant_name, W,
    ...
    print(
        f"[gap_trend_entry_filter] {variant_name} done. "
