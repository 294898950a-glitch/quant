...
def declare_data_requirements(
    ...
    if not data_root_raw:
        raise ValueError(
            "evaluate_cb_arb_signal_blending_volatility_penalized requires --data-root"
        )
    ...
    return {
        "schema_version": 1,
        "executor": "scripts/evaluate_cb_arb_signal_blending_volatility_penalized.py",
...
def _blend_ranking(
    ...
    merged = gap.merge(
        vol, on=["stk_code", "trade_date"], how="left"
    )
...
def _year(
    rows: list[dict[str, Any]], name: str, year: int
) -> dict[str, Any]:
    return next(
        ...
        yaml.safe_dump(
            {
                ...
                "follow_up_actions": [
                    ...
                    "approaches.",
                    "Do not promote unless follow-up review confirms train/2020/test robustness.",
                ],
                "summary": reason,
        ...
        yaml.safe_dump(
            {
                ...
                "q4_monotonic": {
                    ...
                    "answer": "Grid-searched lambda values; no monotonic promotion without manual review.",
                    "computed_data": {
                ...
                "q5_trade_overlap": {
                    ...
                    "answer": "Aggregate train/2020/test checks are used for automatic decision.",
                    "computed_data": {
...
def main() -> int:
    args = _parse_args()
    output_dir = args.output_dir or args.data_root / "signal_blending_volatility_penalized"
    ...
    # Load gap data with stock code mapping (need extra history for vol calculation)
    ...
    vol_stocks = vol_df["stk_code"].nunique()
    vol_dates = vol_df["trade_date"].nunique()
    print(
    ...
    for cfg in configs:
        ...
        summary_rows.append(
            _row(name, description, "train", args.train_start, args.train_end,
                 cfg, params, train)
        )
        summary_rows.append(
            _row(name, description, "test", args.test_start, args.test_end,
                 cfg, params, test)
        )
        ...
        for year in range(2019, 2025):
            ...
            y = _run_value_gap_backtest(
                blended[
                    (blended["trade_date"] >= start) & (blended["trade_date"] <= end)
                ],
                start,
                end,
                args.data_root,
    ...
    best_train = max(train_rows, key=lambda r: float(r.get("score", -999))) if train_rows else {}
    best_test = max(test_rows, key=lambda r: float(r.get("score", -999))) if test_rows else {}
    ...
    #   1. Best composite variant achieves cost-on excess return > 0 over train and sealed test
    #   2. 2020 repair max drawdown < baseline 2020 max drawdown (-0.179) by at least 0.05
    ...
    dd_improvement = selected_2020_dd - baseline_2020_dd if baseline_2020_dd != -999 else 0
