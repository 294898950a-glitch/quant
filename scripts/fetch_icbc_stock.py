"""一次性拉取工商银行 (601398.SH) 日线，落盘到 parquet。

输出: data/icbc_grid/raw/601398_daily.parquet
列: date (str YYYYMMDD), open, high, low, close, vol, amount
    open/high/low/close/amount = float64; vol = int64

akshare 接口尝试顺序:
    1. ak.stock_zh_a_hist(symbol="601398", adjust="qfq", ...)        # 东财, 主路径
    2. ak.stock_zh_a_daily(symbol="sh601398", adjust="qfq")          # 新浪, 带前缀, fallback
失败重试 3 次，每次间隔 0.5s。

注意：工商银行 2006-10 上市，start=20060101 让 akshare 自动从首日返回。
"""
...
OUT_DIR = REPO_ROOT / "data" / "icbc_grid" / "raw"
OUT_PATH = OUT_DIR / "601398_daily.parquet"
