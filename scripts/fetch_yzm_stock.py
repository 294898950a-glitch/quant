"""一次性拉取伊之密 (300415.SZ) 日线，落盘到 parquet。

输出: data/yzm_grid/raw/300415_daily.parquet
列: date (str YYYYMMDD), open, high, low, close, vol, amount
    open/high/low/close/amount = float64; vol = int64

akshare 接口尝试顺序:
    1. ak.stock_zh_a_hist(symbol="300415", adjust="qfq", ...)        # 东财, 主路径
    2. ak.stock_zh_a_daily(symbol="sz300415", adjust="qfq")          # 新浪, 带前缀, fallback
失败重试 3 次，每次间隔 0.5s。

注意：伊之密 2015-04 上市，start=20150101 时 akshare 自动从首日返回。
"""
...
OUT_DIR = REPO_ROOT / "data" / "yzm_grid" / "raw"
OUT_PATH = OUT_DIR / "300415_daily.parquet"
...
def _try_sina() -> pd.DataFrame:
    """新浪日线接口（fallback）。带 sz 前缀。"""
    import akshare as ak

    df = ak.stock_zh_a_daily(symbol="sz300415", adjust="qfq")
    return df
...
def fetch(start: str = START_DATE, end: str | None = None) -> pd.DataFrame:
    ...
    # 2. fallback sina (stock_zh_a_daily, sz 前缀)
    df = _retry(_try_sina)
