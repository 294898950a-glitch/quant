...
# ── Repo root discovery + sys.path for GateKeeper ──────────────────────
...
# in isolated mode from /tmp, so all non-stdlib imports that follow this block
# must resolve from the venv site-packages (numpy/pandas/yaml) or from
# REPO_ROOT (scripts.*).
...
def _ensure_yaml_np_reprs():
    ...
    # No numpy imports in this executor — representer registration is a no-op
    # but kept for compatibility with the standard lazy-import pattern.
...
def _load_data(data_root: str) -> Any:
...
def _compute_persistence_count(df: Any) -> Any:
