...
详见 `docs/research_framework/protocol_redline.md`. 日常只记 7 条:

- 结论必须有数据来源.
- 重活只能在 VM 跑; 是否可跑按预算计算器判断.
- 新研究任务必须有完整设计.
- 超过 10 分钟必须报进度.
- 改变策略真值必须更新 `CURRENT.md` + `baseline_registry.md`.
- 完成回测必须写 `run_manifest`.
- 30 分钟无回复时保持 `当前策略`, 换方向; 预算计算 ≤ ¥100 可自动继续.
...
用户无回复默认规则: 如果系统询问下一步后 30 分钟内没有回复, AI 先读 3 个入口文件填写 `当前策略`, 然后默认保持 `当前策略`, 但换一个研究方向; 不默认归档, 不默认升级实盘, 不继续在刚失败的方向上调参数. 新数据、核心代码、研究预算不再自动卡用户; AI 必须先用 `scripts/estimate_compute_budget.py` 算预算, 预计总额 ≤ ¥100 可继续, 超过 ¥100 或算不出来才等用户.
...
```
🔴 红线 — 不能自动做
   归档当前策略、升级实盘、改协议、复活已确认无效方向、
   把原型提升为当前主策略真值

🟡 研究自动区 — 预算内可做
   新数据、研究脚本、评估脚本、原型策略逻辑、研究参数空间

🟢 普通自动区
   文档、索引、记录、轻量检查、已登记参数值
```
