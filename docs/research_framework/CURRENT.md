...
**最后更新**: 2026-05-16 10:55 CST
**触发更新事件**: 用户新增 30 分钟无回复默认预算规则
...
|---|---|---:|---|---|
| cb_arb 主策略 (verifier yaml-current) | **WIP, 不达门槛** | -12.7% | -30.7% (全段 OOS 2019-2024) | 用户 30 分钟内未回复则默认保持策略、换方向 |
| cb_arb value-gap switch (评估分支) | **WIP 加强版, 不达门槛** | -3.0% | -13.1% (单年 2020) | 用户 30 分钟内未回复则默认保持策略、换方向 |
...
当系统向用户询问下一步, 且 30 分钟内没有回复:

- `当前策略` 不是写死值, 由 AI 读完 3 个入口文件后填写.
- 默认不归档当前策略.
- 默认不升级为实盘策略.
- 默认保持 `当前策略` 继续研究.
- 必须换一个研究方向, 不能继续在刚失败的同一方向上调参数.
- 新方向必须来自 `当前策略` 的当前状态、失败原因和未完成线索.
- 新方向允许新增数据源或改变核心代码, 但必须先写清理由、影响范围和回滚方式.
- 预算由 `scripts/estimate_compute_budget.py` 计算, 不由 AI 主观估.
- 预计总预算 ≤ ¥100 时默认继续; 超过 ¥100 或预算算不出来时, 才停下等用户.

## 下一步 owner (当前等谁)

- **用户决定窗口**: 系统可询问当前策略下一步; 若 30 分钟内无回复, AI 先填写 `当前策略`, 然后保持它并换一个研究方向.
- **arxiv S2 cooldown**: 24h 后可重跑 (但 keywords 需先重审, 否则可能仍 0 通过)
- **Codex**: idle / standby; 若触发 30 分钟默认规则, 先写新方向设计 + 预算计算结果; 预计 ≤ ¥100 可继续执行.
